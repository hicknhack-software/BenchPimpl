...
void Plain(benchmark::State& state)
{
	while (state.KeepRunning())
	{
		auto rect = new PlainRect(1, 2, 3, 4);
		auto sum = rect->width() * rect->height();
...
void SharedPlain(benchmark::State& state)
{
	while (state.KeepRunning())
	{
...
void UniquePimpl(benchmark::State& state)
{
	while (state.KeepRunning())
	{
...
void SharedPimpl(benchmark::State& state)
{
	while (state.KeepRunning())
	{
...
void BenchChainedPimpl(benchmark::State& state)
{
	while (state.KeepRunning())
	{
...
void SharedChainedPimpl(benchmark::State& state)
{
	while (state.KeepRunning())
	{
...
void ChainedDeleterPimpl(benchmark::State& state)
{
	while (state.KeepRunning())
	{
