...
protected:
  ...
  class chained {
    ...
    struct Trait {
      ...
      static Interface*
      interface(void* implementation_ptr) {
      ...
      static void
      destruct_implementation(Interface* interface) {
        p(interface)->~Implementation();
      }
      struct Wrapper : public Interface {
        template <typename... Args>
        Wrapper(Args&&... args) : Interface(std::forward<Args>(args)...) {
        }
      ...
      struct Allocator : public std::allocator<T> {
        ...
        void
        deallocate(typename std::allocator<T>::pointer p, std::size_t n) {
          assert(n == 1);
          Trait::deallocate(p);
        }
  ...
  public:
    ...
    template <typename Interface>
    static typename Interface::Implementation*
    ...
    static void
    destruct_implementation(Interface* interface, Base* base) {
      destruct_implementation(interface);
    ...
    template <typename Interface, typename... Args>
    static typename Trait<Interface>::unique_ptr
  ...
  template <typename Interface>
  static auto
  p(Interface* interface) -> decltype(chained::p(interface)) {
    return chained::p(interface);
  }

  template <typename Interface>
  static auto
  p(const Interface* interface) -> const decltype(chained::p(interface)) {
